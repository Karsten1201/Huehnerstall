...
def create_south_opening_elements(
    ...
    *,
    ...
    """Erzeugt Außentür, Fenster und Tierklappe bündig zu den Rohbauöffnungen.

    Alle Höhen beziehen sich auf Oberkante Bodenplatte (z = 0). ``wall_bottom``
    bleibt aus Kompatibilitätsgründen Bestandteil der Schnittstelle, wird aber
    nicht mehr auf Tür-, Fenster- oder Klappenhöhen aufgeschlagen.
    """
    del wall_bottom
    ...
    # Außentür: Rohbauöffnung beginnt auf Oberkante Bodenplatte.
    door_z = 0.0
    for side, x in (("Left", door_x), ("Right", door_x + door_width - frame_width)):
        _box(document, group, f"DoorFrame_{side}", f"Türzarge {side}", x,
             exterior_y, door_z, frame_width, frame_depth, door_height,
             material="KVH / Rahmenholz", color=wood)
    _box(document, group, "DoorFrame_Top", "Türzarge oben",
         door_x + frame_width, exterior_y, door_z + door_height - frame_width,
         door_width - 2.0 * frame_width, frame_depth, frame_width,
         material="KVH / Rahmenholz", color=wood)
    ...
    for index, z in enumerate((350.0, door_height - 450.0), start=1):
        _box(document, group, f"DoorHinge_{index}", f"Türband {index}",
             door_x + frame_width, exterior_y - leaf_thickness - 6.0, z,
             45.0, 6.0, 120.0, material="Stahl verzinkt", color=metal)

    # Fenster: Unterkante exakt auf der parametrischen Brüstungshöhe.
    window_z = window_sill_height
    frame_parts = (
        ("Left", window_x, window_z, frame_width, window_height),
        ("Right", window_x + window_width - frame_width, window_z,
         frame_width, window_height),
        ("Bottom", window_x + frame_width, window_z,
         window_width - 2.0 * frame_width, frame_width),
        ("Top", window_x + frame_width,
         window_z + window_height - frame_width,
         window_width - 2.0 * frame_width, frame_width),
    )
    for name, x, z, dx, dz in frame_parts:
        _box(document, group, f"WindowFrame_{name}",
             f"Fensterrahmen {name}", x, exterior_y, z, dx, frame_depth, dz,
             material="Holzfensterrahmen", color=wood)

    mullion_x = window_x + window_width / 2.0 - frame_width / 2.0
    _box(document, group, "WindowMullion", "Fenster-Mittelsteg", mullion_x,
         exterior_y - 2.0, window_z + frame_width, frame_width,
         frame_depth + 4.0, window_height - 2.0 * frame_width,
         material="Holzfensterrahmen", color=wood)

    pane_z = window_z + frame_width + clearance
    pane_height = window_height - 2.0 * (frame_width + clearance)
    clear_half_width = (window_width - 3.0 * frame_width) / 2.0
    pane_width = clear_half_width - 2.0 * clearance
    for side, x in (
        ("Left", window_x + frame_width + clearance),
        ("Right", mullion_x + frame_width + clearance),
    ):
        _box(document, group, f"WindowGlass_{side}",
             f"Fensterscheibe {side}", x, exterior_y - glass_thickness,
             pane_z, pane_width, glass_thickness, pane_height,
             material="Isolierglas", color=glass_color)

    # Tierklappe ebenfalls ab Oberkante Bodenplatte.
    flap_z = 0.0
    for side, x in (("Left", flap_x), ("Right", flap_x + flap_width - frame_width)):
        _box(document, group, f"FlapFrame_{side}",
             f"Tierklappenrahmen {side}", x, exterior_y, flap_z,
             frame_width, frame_depth, flap_height,
    ...
    _box(document, group, "FlapFrame_Top", "Tierklappenrahmen oben",
         ...
         material="Rahmenholz", color=wood)
    _box(document, group, "AnimalFlapLeaf", "Tierklappenblatt",
         flap_x + frame_width + clearance, exterior_y - leaf_thickness,
         flap_z + clearance, flap_width - 2.0 * (frame_width + clearance),
         leaf_thickness, flap_height - frame_width - 2.0 * clearance,
    ...
    ramp.Shape = Part.makeBox(ramp_width, ramp_length, ramp_thickness)
    ramp.Placement.Base = App.Vector(
        flap_x + frame_width,
        exterior_y - ramp_length,
        -ramp_thickness,
    )
